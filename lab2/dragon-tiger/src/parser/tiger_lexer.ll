...
{integer} 	
	{char * pEnd;
	long int integer = strtol(yytext, &pEnd, 10);

	if (integer <= TIGER_INT_MAX || integer >= - (TIGER_INT_MAX + 1)){
		return yy::tiger_parser::make_INT(integer, loc);
	}
	else {
		utils::error(loc, "Integer is not in the accepted range");
	}
	}
